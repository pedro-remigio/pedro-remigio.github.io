...
function renderizarEventos(lista) {
  ...
  lista.forEach((evento) => {
    ...
    const titulo   = escaparHTML(evento.titulo);
    const descricao = escaparHTML(evento.descricao);
    const local    = escaparHTML(evento.local);
    const imgSrc   = escaparHTML(imagem);
    ...
    eventosContainer.innerHTML += `
      <article class="bg-white rounded-xl shadow-lg overflow-hidden hover:scale-105 transition duration-300 flex flex-col">
        <a href="evento.html?id=${id}">
          <img src="${imgSrc}" alt="${titulo}" class="w-full h-56 object-cover hover:opacity-90 transition">
        </a>
        <div class="p-6 flex flex-col flex-1">
          <div class="flex items-start justify-between gap-2 mb-2">
            <a href="evento.html?id=${id}" class="hover:text-blue-600 transition">
          ...
          <p class="text-gray-500 text-sm mb-3 line-clamp-2">${descricao}</p>
          <p class="text-gray-600 text-sm mb-1">📍 ${local}</p>
          <p class="text-gray-600 text-sm mb-1">📅 ${formatarData(evento.data)}</p>
          <p class="text-sm mb-4">💰 ${precoHtml}</p>
          <div class="mt-auto">
            <a href="evento.html?id=${id}" class="inline-block bg-blue-600 hover:bg-blue-700 transition text-white px-5 py-3 rounded-lg font-semibold text-sm">
              Ver detalhes →
            </a>
          </div>
        </div>
